...
### G39. Marketplace screen with custom screen-level navigation (2026-05-13)

Introduces screen-level navigation: `useScreenStore` (Zustand, session-scoped — not persisted) toggles between `'camera'` and `'marketplace'`. `App.tsx` switches between `CameraScreen` and `MarketplaceScreen` based on this state. `useActivePoseIds` (MMKV-persisted under id `'pose-library'`, key `'poseLibrary.activeIds.v1'`) controls which poses appear in the Camera carousel — default 7 active (`casual-standing-01`, `tpose`, `hands-hips`, `arm-up-right`, `power-stance`, `casual-lean`, `squat`), 3 in Marketplace only until added (`crosslegged`, `warrior-1`, `thinker`). `ALL_POSE_IDS` is derived from `poses.generated.json` at module load so the source-of-truth stays the generated library, not a hardcoded constant. Load is defensive: unknown IDs in serialized state are dropped on load (forward-compat with library reshuffles); a malformed JSON payload silently falls back to defaults.

`MarketplaceScreen` displays all 10 poses with reference photo thumbnails (80×100, via the existing `poseImageAssetMap.getPoseImage`) and per-pose pills: greyed `✓ In your carousel` for active poses (tap removes), purple `+ Add to carousel` for inactive (tap adds). A minimum of 1 active pose is enforced via a pure helper `toggleActivePoseId(current, poseId)` returning `null` when the toggle would leave zero active or when the pose ID isn't in `ALL_POSE_IDS`. When the user is at the minimum and the active row is highlighted, an inline `"At least 1 pose required"` hint appears under the row.

Filter applies ONLY to `PoseSelector`'s library tail (the otherwise unsectioned card list at the end of the carousel). For You / AI Picks / custom captures bypass the filter — their value is contextual (scene-driven or explicitly user-saved) and silencing them via the user's library preference would break the recommendation contract. If the AI recommends `warrior-1` while the user has it in Marketplace-only state, the recommendation still surfaces; the user can tap it directly without going to Marketplace.

Navigation approach is custom (a 12-line Zustand store) rather than React Navigation, to avoid pulling in `react-native-screens` as a new native dependency and triggering a debug APK rebuild (per G31's no-prebuild constraint). Acceptable for 2 screens; upgrade to React Navigation when the app reaches ~4+ screens. The inactive screen is unmounted (not `display: 'none'`) so the camera releases native resources when the user navigates to Marketplace; cold re-init when returning to camera is ~1–2 s on Samsung A22 5G, acceptable for v1. Camera permission is remembered across the unmount cycle so users do not see a re-prompt.
